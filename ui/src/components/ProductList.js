...
function ProductItem({ product }) {
  ...
  const [deleteProductDialogShown, setDeleteProductShown] = useState(false);
  ...
  return (
    <>
      <Dialog
        isShown={deleteProductDialogShown}
        title="Delete product"
        intent="danger"
        onCloseComplete={() => setDeleteProductShown(false)}
        ...
        >
          ...
          <Pane alignItems="flex-end" display="flex" padding={majorScale(2)}>
            ...
            <TrashIcon
              size={14}
              title="Delete product"
              transition="transform 0.15s ease-out 0.15s, opacity 0.1s ease-out 0.25s"
              transform={isHovered ? 'translateY(0)' : 'translateY(40px)'}
